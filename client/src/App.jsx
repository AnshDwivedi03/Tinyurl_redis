...
// --- Axios Interceptor to Handle 401/403 (Auto-Logout) ---
  ...
  (error) => {
    if (error.response && (error.response.status === 401 || error.response.status === 403)) {
      if (localStorage.getItem('token')) {
        ...
        // Optional: Force reload to clear state if not using a global store/context fully
...
// Assuming redirect base is the root of the API domain or separate. 
// For Vercel same-domain deployment, it might just be the root URL. 
// But keeping it flexible:
const REDIRECT_BASE = import.meta.env.VITE_REDIRECT_BASE_URL || 'http://localhost:5000';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      interval = setInterval(() => {
        fetchUrls();
        fetchTrending(); // Optional: update trending too
      }, 5000);
    }
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-900 text-white">
        <div className="relative">
          <div className="h-16 w-16 rounded-full border-4 border-slate-700 border-t-blue-500 animate-spin"></div>
          <div className="absolute inset-0 flex items-center justify-center">
            <Link2 className="h-6 w-6 text-blue-500 animate-pulse" />
          </div>
        </div>
        <p className="mt-4 text-slate-400 font-medium animate-pulse">Initializing NanoLink...</p>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="fixed inset-0 bg-slate-900 flex items-center justify-center p-4 font-sans z-50">
