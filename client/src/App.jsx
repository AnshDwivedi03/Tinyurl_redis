...
const App = () => {
  ...
  const handleShorten = async (e) => {
    ...
    setLastLatency(null);
    try {
      const { data } = await axios.post(`${API_BASE}/url/shorten`, {
        originalUrl: newUrl,
      });
      setNewUrl("");
      fetchUrls();
    ...
    return (
      <div className="min-h-screen w-full bg-slate-900 flex items-center justify-center p-4 font-sans">
        <div className="bg-slate-800 p-8 rounded-2xl w-full max-w-md border border-slate-700 shadow-2xl">
          ...
          {error && (
            <div className="bg-red-500/10 text-red-400 p-3 rounded mb-4 text-center border border-red-500/20">
              {error}
            </div>
          )}
        ...
        <header className="flex justify-between items-center mb-10 bg-slate-800/50 p-4 rounded-xl border border-slate-700/50 backdrop-blur-sm sticky top-4 z-10">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight">
              NanoLink<span className="text-blue-500">Pro</span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-white hover:bg-slate-700/50 px-3 py-2 rounded-lg transition"
            >
              <LogOut size={18} />{" "}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl">
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
                <Activity className="text-blue-400 w-5 h-5" /> Create Short Link
              </h2>
              <form
              ...
              >
                <input
                  ...
                  className="flex-1 bg-slate-900 border border-slate-700 rounded-xl p-4 text-white focus:ring-2 focus:ring-blue-500 outline-none transition"
                  value={newUrl}
                  onChange={(e) => setNewUrl(e.target.value)}
                />
                <button className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-xl font-bold transition shadow-lg shadow-blue-600/20 whitespace-nowrap">
                  Shorten URL
                </button>
              </form>
              {lastLatency && (
                <div className="mt-4 inline-flex items-center gap-2 bg-green-500/10 text-green-400 px-4 py-2 rounded-full text-sm border border-green-500/20 animate-in fade-in slide-in-from-top-2">
                  <Zap size={14} className="fill-green-400" /> Processed in{" "}
            ...
            <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-xl">
              <div className="p-4 border-b border-slate-700 font-semibold bg-slate-800/80 backdrop-blur-md">
                Your Active Links
              </div>
              {urls.length === 0 ? (
                ...
                <div className="divide-y divide-slate-700">
                  {urls.map((url) => (
                    <div
                      key={url._id}
                      className="p-4 hover:bg-slate-700/30 transition group flex items-center justify-between"
                    >
                      <div className="flex-1 min-w-0 pr-4">
                        <div className="flex items-center gap-2 mb-1">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-blue-400 font-bold text-lg tracking-tight hover:underline flex items-center gap-1"
                          >
                            {REDIRECT_BASE}/{url.shortCode}{" "}
          ...
          <div className="space-y-8">
            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                <TrendingUp className="text-purple-400 w-5 h-5" /> Global
                Trending
              </h3>
                  ...
                  trending.map((t, i) => (
                    ...
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <a
                          href={`${REDIRECT_BASE}/${t.shortCode}`}
                          target="_blank"
                          rel="noreferrer"
                          className="text-slate-300 hover:text-blue-400 transition truncate block"
                        >
                          {REDIRECT_BASE}/{t.shortCode}
                        </a>
                      </div>
                      <span className="bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded text-xs font-mono">
                        {t.clicks}
                      </span>
                    </div>
                  ))
